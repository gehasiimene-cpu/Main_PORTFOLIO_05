...
# Compatibility: simple icons shim for Flet versions lacking ft.icons
...
def main(page: ft.Page):
    ...
    def home(e):
        ...
        slide_view = ft.Container(
            content=slide_pages[slide_index[0]],
            width=700,
            height=240,
        )

        def update_slide_view():
            slide_view.content = slide_pages[slide_index[0]]
            slide_status.value = f"Slide {slide_index[0] + 1} / {len(slide_pages)}"
            page.update()
        ...
        def next_slide(e):
            if slide_index[0] < len(slide_pages) - 1:
                slide_index[0] += 1
                update_slide_view()
    ...
    def blog(e):
        ...
        def technical_blog_video_card():
            ...
            video_controls.append(
                ft.FilledButton(
                    "Open Video",
                    style=ft.ButtonStyle(bgcolor=ACCENT_COLOR, color="#FFFFFF"),
                    on_click=open_video,
                )
            )
        ...
        content=ft.Column(
            controls=[
                ft.Row([
                    ...
                    ft.Row([
                        ft.IconButton(ft.Icons.SWAP_HORIZ, icon_color=PRIMARY_COLOR, on_click=move_chat, tooltip="Move chat"),
                        ft.IconButton(ft.Icons.CLOSE, icon_color=PRIMARY_COLOR, on_click=close_chat, tooltip="Minimize chat"),
                    ], spacing=0),
