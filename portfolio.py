...
# Compatibility: some Flet versions expose BoxFit, others expose ImageFit,
# and older/newer versions may use string values. Resolve a safe value here.
...
# Provide a lightweight PageView fallback for Flet versions that don't expose it.
if not hasattr(ft, "PageView"):
    class _CompatPageView(ft.Container):
        def __init__(self, controls=None, selected_index=0, horizontal=False, keep_page=False, snap=False, pad_ends=False, width=None, height=None, on_change=None, **kwargs):
            # Use a Row to hold pages; show all but track selected_index for logic
...
def main(page: ft.Page):
    ...
    def make_link_handler(url):
        def handler(e):
            if hasattr(page, "launch_url"):
                page.launch_url(url)
            elif hasattr(page, "launch_url_async"):
                page.run_task(page.launch_url_async, url)
            else:
                # Local fallback for older Flet versions.
                threading.Thread(target=webbrowser.open_new_tab, args=(url,), daemon=True).start()
        return handler

    def make_mailto_handler(name_field, email_field, msg_field):
        def handler(e):
            subject = quote(f"Message from {name_field.value or 'Portfolio visitor'}")
            body = quote(
                f"From: {email_field.value or ''}\n\n{msg_field.value or ''}",
                safe="",
            )
            make_link_handler(f"mailto:gehasiimene@gmail.com?subject={subject}&body={body}")(e)
        return handler
    ...
    def home(e):
                                    ...
                                    content=ft.Column(
                                        ...
                                        controls=[
                                            ...
                                            ft.FilledButton(
                                                ...
                                                width=float("inf"),
                                                on_click=make_mailto_handler(name_field, email_field, msg_field)
                                            ),
                                        ]
                                    ),
                                ),
        ...
        main_content.controls.append(home_container)
    ...
    def blog(e):
        ...
        def technical_blog_video_card():
            ...
            video_src = asset_path(video_name)
            video_file = local_asset_path(video_name)

            def open_video(e):
                make_link_handler(video_src)(e)
            ...
            if video_file.exists() and hasattr(ft, "Video") and hasattr(ft, "VideoMedia"):
                video_controls.append(
                    ft.Video(
                        playlist=[ft.VideoMedia(video_src)],
                        aspect_ratio=16 / 9,
                        autoplay=False,
                        muted=False,
                    )
...
if __name__ == "__main__":
    # Start the Flet app (support multiple Flet API variations)
    # Some Flet releases expose `app()` while others provide `run()`.
    # Try the common variants in a safe order.
    ...
    if hasattr(ft, "app"):
        try:
            ...
            try:
                ...
                # fallback to positional argument signature
                ft.app(main)
    elif hasattr(ft, "run"):
        try:
            ft.run(**app_kwargs)
        ...
        raise RuntimeError("Flet API not found: please upgrade/downgrade the 'flet' package to a compatible version.")
